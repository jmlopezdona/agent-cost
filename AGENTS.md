...
7. **URL como única persistencia de escenario**: cualquier parámetro nuevo del escenario debe añadirse a `PARAMS` en `src/store/urlSync.ts` (clave corta, solo se serializan diffs frente al preset base, escritura con `replaceState`). Añade el round-trip a `urlSync.test.ts`. Única excepción de persistencia: el tema en `localStorage`.
