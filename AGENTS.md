...
3. **Caso dorado intocable**: el preset P2 (régimen 12×5, duty 0,6) debe producir blend ≈ 13,8 $/h, techo ≈ 3.585 $/mes y ponderado ≈ 2.151 $/mes con error < 1% (tests en `src/engine/engine.test.ts`). El blend (≈ 13,8 $/h) depende solo de la mezcla y los precios; techo y ponderado se derivan del horario (260 h/mes) y el duty. Si cambias precios, presets o fórmulas y estos tests fallan, el cambio es incorrecto o exige actualizar el PRD primero.
