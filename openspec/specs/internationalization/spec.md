...
Interfaz multilingüe en español (`es`, locale canónico), inglés (`en`) y francés (`fr`) con selector en la cabecera, resolución reactiva del idioma activo desde un único store, autodetección inicial con fallback inglés, persistencia local (sin contaminar la URL del escenario) y prosa de presets y salarios resuelta desde i18n por id.
