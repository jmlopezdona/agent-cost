...
Persistencia del escenario por pestaña en `sessionStorage`: el escenario y los modificadores de configuración avanzada se guardan automáticamente con el mismo formato compacto que la URL de compartir, sobreviven al refresco de página y son independientes entre pestañas. Define la precedencia de un enlace entrante sobre el estado almacenado y el botón "Reset" que devuelve la aplicación al preset por defecto.
